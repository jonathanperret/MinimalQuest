...
partial class OculusBuildApp : EditorWindow
{
...
#if UNITY_EDITOR && UNITY_2018_3_OR_NEWER && UNITY_ANDROID
	...
	[MenuItem("Oculus/OVR Build And Run %&b", false, 10)]
					...
					e.Data.Length != 0 && e.Data.Contains("BUILD"))
				{
					...
					if (e.Data.Contains("SUCCESSFUL"))
					{
						UnityEngine.Debug.LogFormat("APK Build Completed: {0}",
							Path.Combine(gradleProjectPath.Replace('/', Path.DirectorySeparatorChar), "build", "outputs", "apk", "debug", productName + "-debug.apk"));
	...
	public static bool DeployAPK()
	{
		...
		if (adbTool.isReady)
		{
			...
			gradleExportFolder = gradleExportFolder.Replace('/', Path.DirectorySeparatorChar);
